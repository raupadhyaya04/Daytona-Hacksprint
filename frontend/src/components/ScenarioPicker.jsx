...
export default function ScenarioPicker({ scenarios, value, onChange, onLaunch, launching }) {
  // Defensive: backend might return an array, {scenarios: [...]}, {data: [...]},
  // or nothing yet while the fetch is in flight — never let this crash the page.
  const list = Array.isArray(scenarios)
    ? scenarios
    : Array.isArray(scenarios?.scenarios)
    ? scenarios.scenarios
    : Array.isArray(scenarios?.data)
    ? scenarios.data
    : [];
